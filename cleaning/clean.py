...
# matches
matches_path = Path(__file__).resolve().parents[1] / "datasets" / "matches.csv"
...
unique_matches['white_rtg'] = unique_matches['white_id'].astype('Int64')
unique_matches['black_rtg'] = unique_matches['white_id'].astype('Int64')

unique_matches.to_csv(Path(__file__).resolve().parents[1] / "datasets" / "unique_matches.csv", index=False)

# players
players_path = Path(__file__).resolve().parents[1] / "datasets" / "players.csv"
...
unique_players.to_csv(Path(__file__).resolve().parents[1] / "datasets" / "unique_players.csv", index=False)

# tournaments
tournament_path = Path(__file__).resolve().parents[1] / "datasets" / "tournaments.csv"
...
unique_tournaments.to_csv(Path(__file__).resolve().parents[1] / "datasets" / "unique_tournaments.csv", index=False)